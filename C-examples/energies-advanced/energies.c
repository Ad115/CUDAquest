...
The energy profile is the energy values of sliding windows. A sliding window's 
energy is the sum of the energy values of the dinucleotides in the window. This
can be used to predict the promoting sequences locations.

To use it, compile with:

    cc energies.c -o energies

Run it with:

    ./energies -i sequence.fasta -o out.txt
    
Or, to see it graphically:

    ./energies -i sequence.fasta | energies.py
*/
...
int main(int argc, char *argv[]) {
    ...
    // Fetch the input & output files
    char *input = findInArguments(args, "i|in");
    
    char *output = findInArguments(args, "o|out");
    
    // Open the output file
    FILE *out = openOutputFile( output );
        
    // 2. ---> Calculate the energy profile of the sequence
    
    if ( fileExists(input) ) { // Validate files
        
        char *sequence = getFASTASequence(input);
...
FILE *openOutputFile(char *output) {
/*
 * Tries to open a file named `òutput` and if it fails
 */
